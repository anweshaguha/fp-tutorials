...
AW: I think it would be great to add a demonstration of how the error message ends up presenting. I added a code chunk below testing it out.

```{r simulate}

# This is how I am generating data without a loop. I can loop the sample specification
# across 5 variables and vary the probability weights. Parallel looping?
...
likert_sim <- function(max, n, weights, levels, labels) {
  ...
  stop("Are the length of your labels, levels, or weights equal to the likert item specified? ",  
          "You specified a ", max, " point likert item scale. Did you add sufficient arguments? ",
         "Number of weight arguments added was ", length(weights), #AW: The comma before this "Number" could be removed
         ", Number of labels arguments added was ", length(labels),
         ", Number of level arguments added was ", length(levels))
  }
}

#AW: Nice error message! I made a few small suggested edits. I think you could also use periods after each "Number...." statement.
```
...
#AW: testing your error message
levels2 <- c(1, 2, 3)
labels2 <- c("Strongly disagree", "Disagree", "Agree", "Strongly agree")

q_err <- likert_sim(4, 250, c(.20, .30, .25, .25), levels2, labels2)
```
...
AW: Sorry I didn't have time to help trouble-shoot this. It definitely seems like something that could be streamlined with a function though.
...
response ID. The data is now ready to be graphed!
...
we will break the task and create two separate functions, and then apply those two functions
...
AW: I didn't have time to help trouble-shoot this either, sorry! 
...
graph_likert <- function(df) {
  ...
  geom_text(aes(label = abs(percent_answers)), #AW: these are nice!
            position = position_stack(vjust = 0.5),
            color = "white",
            fontface = "bold") +
  coord_flip() +
  scale_x_discrete() +
  scale_fill_viridis_d() + #AW: I like the color scale you chose!
  labs(title = "To what extent do you agree with the following statements?",
       y = "Percent of respondents", #AW: I think this should be on the y-axis since you're doing the coord_flip
       fill = NULL) +
  theme_minimal() +
  theme(axis.text.x = element_blank(),
        axis.title.y = element_blank(),
        panel.grid = element_blank(),
        legend.position = "bottom") #AW: suggest matching the ordering of the colors on the legend to how they appear (yellow, green, blue, purple)
}
