...
I am looking at the hashtag "CRT" or culturally relevant pedagogy. Note: the Twitter API only returns tweets from the last 6-9 days. As a result, the 18,000-tweet request was not met; only 1,567 tweets exist for that time window.
...
Say we wanted to create models for each of these groups. One way could be to create a data frame for each group and fit a linear model. 
...
sentiment$hour <- format(round(sentiment$created_at, units = "hours"), 
                         format = "%H:%M")
```
