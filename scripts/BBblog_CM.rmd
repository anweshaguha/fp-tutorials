...
Next we need to set up a directory for the csv files that we will be using. Note, I am specifically showing you how to set a directory rather than using the here function, but all modified files are available on our github site and at the original files at the link above.
```{r}
BBdata_dir <- "/Users/cassiemalcom/Desktop/CM2020/22_Spring Term Classes/EDLD 653_DS3/fp-tutorials/Lego"
```
...
Second, we can use the tally function to find the total number of parts when you consider color as a part variation. We can use this number later to test if the 
first function we are about to write is working correctly.
```{r}
count_colors %>%
  tally(n)
```
